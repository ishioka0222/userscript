...
- ルールごとにラベルの文字色 / 背景色 / 位置を指定できます。
- タブタイトルに `[本番] ` のような接頭辞を付けたり、画面の縁に色枠を出したりできます（ルールごとに on / off）。
...
| この URL のルールを追加…     | 現在の URL を元に pattern・ラベル・背景色を入力して `rules` に追加します |
...
「追加」ボタンを押し、ラベル名と背景色を入力すると、捕捉した値を `match` に入れた instance が自動で追加されます。
...
| ラベル共通（`instances[]` / `rules[]`） | `label`（必須）、`bg`（背景色、既定 `#d32f2f`）、`fg`（文字色、省略時は背景の明るさから自動）、`position` / `titlePrefix` / `frame`（省略時は `defaults`） |
