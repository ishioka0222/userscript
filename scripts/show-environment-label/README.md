...
「追加」ボタンを押すとフォームが開き、名前と環境（環境を使わない場合は色）を入力して追加すると、捕捉した値を `match` に入れた instance が自動で追加されます。
...
- CSP の `sandbox`（`allow-modals` なし）で `alert` / `prompt` が無効化されているページ（例: Db2 管理コンソール）でも動くよう、入力フォームと通知（トースト）も Shadow DOM 内の自前 UI で行い、`alert` / `prompt` は使いません。
