...
| [Copy Page Link](scripts/copy-page-link/)                                                                      | 任意のページのタイトルと URL を、テキスト / Markdown / リッチテキストのリンクとしてコピーするコマンドを Tampermonkey のメニューに追加します。                                            | すべてのサイト                                                          | [Install](https://raw.githubusercontent.com/ishioka0222/userscript/master/scripts/copy-page-link/copy-page-link.user.js)                                                           |
...
| [Show Environment Label](scripts/show-environment-label/)                                                      | URL がルールに一致するページに「本番」「開発」などのラベルを常時表示します。設定はローカルのみで外部通信なし。                                                                           | すべてのサイト                                                          | [Install](https://raw.githubusercontent.com/ishioka0222/userscript/master/scripts/show-environment-label/show-environment-label.user.js)                                           |
| [Save Page as Markdown](scripts/save-page-as-markdown/)                                                        | 表示中のページの本文（または選んだ要素）を Markdown に変換して .md ファイルとしてダウンロードします。                                                                                    | すべてのサイト                                                          | [Install](https://raw.githubusercontent.com/ishioka0222/userscript/master/scripts/save-page-as-markdown/save-page-as-markdown.user.js)                                             |
...
- `<name>` は `<サイト識別子>-<ユーザーから見た効果>` の kebab-case で命名します（例: `udemy-skip-next-lecture-countdown`）。すべてのサイトで動く汎用スクリプトはサイト識別子を付けず `<効果>` のみにします（例: `copy-page-link`）。
